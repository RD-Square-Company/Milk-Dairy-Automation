...
EPaymentUpdate::EPaymentUpdate(QString empID, QString emailID, QWidget *parent)
    : QDialog(parent)
{
    this->custID = empID;
    this->emailID = emailID;

    //setting ui to paymentupdate
    setupUi(this);
...
void EPaymentUpdate::setWidgets()
{
    //setting title label
    titleLabel->setText(tr("<html><center><p><b>Payment Information of</b> <i>%1</i></p><center></html>").arg(custID));
...
void EPaymentUpdate::accountTypeChanged(int type)
{
    if(type == 0) {
        noaLabel->hide();
        updateButton->hide();
...
void EPaymentUpdate::updatePressed()
{
    rLoad(1);
    QString error;
    ...
    if(error.isEmpty()){
        ...
        queryString =  "select BankInfo.NameOnAccount "
                       "from BankInfo, BankAccountNo "
                       "where DCAccount = '" + aNumberEdit->text() + "' and "
                                                                     "binary BankInfo.BID = BankAccountNo.BID and "
                                                                     "BankInfo.ACType = '" + acTypeBox->currentText() + "'; ";
        ...
        if(query.size()> 0){
            ...
            if(saveData()){
                ...
                OfflineEmpEdit::editPayment(this->custID, this->eID, noaEdit->text(),aNumberEdit->text(),
                                            acTypeBox->currentText(), bNameEdit->text(), ifscEdit->text(),
                                            bAddEdit->text(), cvvEdit->text(), QString());
                this->eID = "";
                return;
            } else {
                rLoad(0);
...
void EPaymentUpdate::sendMail()
{
    QString subject = tr("Update Successfull");
    QString message = tr("Dear employee,\n\nYour account information is updated successfully by secratery of the dairy. Please login to your MDS account to see your information.\n\n"
    ...
    smtp->sendMail(oDetail.getHostEmail(), this->emailID, subject, message);

}

void EPaymentUpdate::showMessage()
{
    rLoad(0);
    disconnectDatabase();
    QMessageBox::information(this,
...
void EPaymentUpdate::cancelPressed()
{
    paymentBox->hide();
    okButton->show();
    deleteButton->show();
...
void EPaymentUpdate::closeEvent(QCloseEvent *)
{
    disconnectDatabase();
}

void EPaymentUpdate::rLoad(int i)
{
    if(i ==1){
...
void EPaymentUpdate::okPressed()
{
    this->close();
}

void EPaymentUpdate::sendSignal()
{
    disconnectDatabase();
    rLoad(0);
    QMessageBox::information(this,
...
void EPaymentUpdate::newPressed()
{
...
bool EPaymentUpdate::connectDatabase()
{
...
void EPaymentUpdate::disconnectDatabase()
{
    myDatabase = QSqlDatabase();
    myDatabase.removeDatabase("remoteDBase");
}

void EPaymentUpdate::setTableValue()
{
    if(!myDatabase.isOpen()){
        if(!connectDatabase()){
            deleteButton->hide();
    ...
    QSqlQuery query(myDatabase);
    QString queryString;
    queryString = " select BankInfo.BID, BankInfo.ACType, BankInfo.NameOnAccount, BankAccountNo.DCAccount, BankInfo.BankName, "
                  "BankInfo.IFSC_Code, BankInfo.BranchName, BankInfo.CVV, BankInfo.Expdate "
                  " from BankInfo, BankAccountNo, EmployeeAccount "
                  "where binary BankInfo.BID = BankAccountNo.BID and "
                  "binary BankInfo.BID = EmployeeAccount.BID and "
                  "binary EmployeeAccount.EmpID = " + this->custID + " and "
                                                                     "binary BankInfo.LocalDID = '" + mDetail.getDID() + "';";
    retry:
    if(!query.exec(queryString)){
        int result = QMessageBox::critical(this,
                              tr("Connection Error! : %1").arg(qApp->applicationName()),
    ...
    if(query.size() > 0){
        ...
        while(query.next()){
            ...
            item->setText(query.value("DCAccount").toString());
...
void EPaymentUpdate::deletePressed()
{
    rLoad(1);
    if(!myDatabase.isOpen()){
        if(!connectDatabase()){
    ...
    for(int row = 0; row<checkList.count();row++){
        if(checkList.at(row)->isChecked()){
            count = 1;
            queryString  += "delete from EmployeeAccount where binary EmpID = " + this->custID + " and "
                                                                                                                     " binary BID = '" + checkList.at(row)->text() + "';";
            queryString += "delete from BankAccountNo where binary BID = '" + checkList.at(row)->text() + "';";
            queryString += "delete from BankInfo where binary BID = '" + checkList.at(row)->text() + "' and  "
    ...
    if(!query.exec(queryString)){
        ...
        for(int row = 0; row<checkList.count();row++){
            if(checkList.at(row)->isChecked()){
                OfflineEmpEdit::removePayment(this->custID, checkList.at(row)->text());
            }
        }
    }
}

bool EPaymentUpdate::saveData()
{
    QString bankQuery, accountQuery, custAccountQuery;
    bankQuery = "INSERT INTO BankInfo(BID,NameOnAccount,ACType,BankName,IFSC_Code,BranchName,CVV,Expdate, LocalDID)";
    accountQuery = "insert into BankAccountNo (BID, DCAccount) ";

    custAccountQuery = "insert into EmployeeAccount (EmpID, BID) ";
    ...
    if(acTypeBox->currentIndex() != 0){
        ...
        //insert values into produceraccount or consumeraccount

        custAccountQuery += " values (" + this->custID + "," + QString::number(bid) + ");";

        QString fullQuery = "START TRANSACTION; " +
                bankQuery + accountQuery + custAccountQuery +
                "COMMIT;";
...
void EPaymentUpdate::paintEvent(QPaintEvent *event)
{
