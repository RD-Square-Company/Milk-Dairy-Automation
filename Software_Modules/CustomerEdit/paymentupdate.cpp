...
PaymentUpdate::PaymentUpdate(QString type, QString custID,QString emailID ,QWidget *parent)
    : QDialog(parent)
{
    this->type = type;
    this->custID = custID;
    ...
    if(error.isEmpty()){
        ...
        if(query.size()> 0){
            ...
            if(saveData()){
                ...
                OfflineCustEdit::editPayment(this->custID, this->bID, noaEdit->text(), aNumberEdit->text(),
...
void PaymentUpdate::sendMail()
{
    ...
    QString message = tr("Dear %1,\n\nYour account information is updated successfully by secratery of the dairy. Please login to your MDS account to see your information.\n\n"
              "Thank You.\nFor more support and information, visit our website www.mds.onlinehelp.co.on or call us on +91 7568577487.").arg(this->custID);
...
bool PaymentUpdate::saveData()
{
    ...
    if(this->type == "Producer"){
        custAccountQuery = "insert into ProducerAccount (ProdID, BID) ";
    } else {
        custAccountQuery = "insert into ConsumerAccount (ConsID, BID) ";
    }
...
void PaymentUpdate::deletePressed()
{
    ...
    if(this->type == "Producer"){
        for(int row = 0; row < checkList.count(); row++){
            if(checkList.at(row)->isChecked()){
                count = 1;
                queryString  += "delete from ProducerAccount where binary ProdID = '" + this->custID + "' and "
        ...
        for(int row = 0; row < checkList.count(); row++){
            if(checkList.at(row)->isChecked()){
                count = 1;
                queryString  += "delete from ConsumerAccount where binary ConsID = '" + this->custID + "' and "
                                                                                                                         " binary BID = '" + checkList.at(row)->text() + "';";
                queryString += "delete from BankAccountNo where binary BID = '" + checkList.at(row)->text() + "';";
                queryString += "delete from BankInfo where binary BID = '" + checkList.at(row)->text() + "' and  "
                               " binary LocalDID = '" + mDetail.getDID() + "';";
    ...
    if(!query.exec(queryString)){
        ...
        for(int row = 0; row < checkList.count(); row++){
            if(checkList.at(row)->isChecked()){
                OfflineCustEdit::removePayment(this->custID, checkList.at(row)->text());
            }
        }
    }
}
...
{
    if(!myDatabase.contains("custPaymentEdit")){
        myDatabase = QSqlDatabase::addDatabase("QMYSQL", "custPaymentEdit");
...
void PaymentUpdate::disconnectDatabase()
{
    ...
    myDatabase.removeDatabase("custPaymentEdit");
}
...
void PaymentUpdate::setTableValue()
{
    ...
    if(this->type == "Producer"){
        queryString = " select BankInfo.BID, BankInfo.ACType, BankInfo.NameOnAccount, BankAccountNo.DCAccount, BankInfo.BankName,"
                      ...
                      " from BankInfo, BankAccountNo, ProducerAccount "
                      "where binary BankInfo.BID = BankAccountNo.BID and "
                      " binary BankInfo.BID = ProducerAccount.BID and "
                      " binary ProducerAccount.ProdID = '" + this->custID + "' and "
                                                                            " binary BankInfo.LocalDID = '" + mDetail.getDID() + "' ;";
    } else {
        queryString = " select BankInfo.BID, BankInfo.ACType, BankInfo.NameOnAccount, BankAccountNo.DCAccount, BankInfo.BankName,"
                      "BankInfo.IFSC_Code, BankInfo.BranchName, BankInfo.CVV, BankInfo.Expdate "
                      " from BankInfo, BankAccountNo, ConsumerAccount "
                      "where binary BankInfo.BID = BankAccountNo.BID and "
                      " binary BankInfo.BID = ConsumerAccount.BID and "
                      " binary ConsumerAccount.ConsID = '" + this->custID + "' and "
                                                                            " binary BankInfo.LocalDID = '" + mDetail.getDID() + "' ;";
    }
