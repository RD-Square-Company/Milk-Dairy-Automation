    ...
    if(connectDatabase()){
       ...
       QSqlDatabase::database("remoteDBase").transaction();
        ...
        emp += QString::number(empID) + ", '" + registrationDialog->firstNameEdit->text() + "', '" + registrationDialog->middleNameEdit->text() + "','" +
                ...
                registrationDialog->emailEdit->text() + "'," + registrationDialog->salaryEdit->text()+ ", '" + mDetail.getDID() + "');";
        ...
        if(paymentDialog->acTypeBox->currentIndex() != 0) {
            ...
            if(!query.exec(allQuery)){
               ...
               QSqlDatabase::database().commit();
                ...
                QTimer::singleShot(2000, this, SLOT(completionMessage()));
                OfflineEmpRegistration::registerOffline(eID,"1",registrationDialog->ad1Edit->text()+registrationDialog->ad2Edit->text()+registrationDialog->ad3Edit->text(),
                                                        registrationDialog->firstNameEdit->text(), registrationDialog->middleNameEdit->text(), registrationDialog->lastNameEdit->text(),
                                                        registrationDialog->pContactEdit->text(), registrationDialog->sContactEdit->text(), registrationDialog->line1Edit->text(),
                                                        registrationDialog->line2Edit->text(), registrationDialog->stateBox->currentText(), registrationDialog->pincodeEdit->text(),
                                                        registrationDialog->emailEdit->text(), registrationDialog->dojEdit->date().toString("yyyy-MM-dd"), registrationDialog->dobEdit->date().toString("yyyy-MM-dd"),
                                                        registrationDialog->hQBox->currentText(), registrationDialog->salaryEdit->text(), value, registrationDialog->designationEdit->text(),
                                                        bID, paymentDialog->noaEdit->text(),paymentDialog->aNumberEdit->text(), paymentDialog->acTypeBox->currentText(), paymentDialog->bNameEdit->text(),
                                                        paymentDialog->ifscEdit->text(),paymentDialog->bAddEdit->text(), "", "");
                bID = "";
            }
            return;
        }
        ...
        if(!query.exec(allQuery)){
           ...
           QSqlDatabase::database("remoteDBase").commit();
            ...
            if(!registrationDialog->emailEdit->text().isEmpty()){
                sentMail(registrationDialog->emailEdit->text(), registrationDialog->firstNameEdit->text() + " " + registrationDialog->lastNameEdit->text(),
                         registrationDialog->departmentBox->currentText(), registrationDialog->designationEdit->text(), registrationDialog->salaryEdit->text());
                //expire date is set to null because temporarily this feature (credit and debit) is removed from system
                QTimer::singleShot(2000, this, SLOT(completionMessage()));
                OfflineEmpRegistration::registerOffline(eID,"1",registrationDialog->ad1Edit->text()+registrationDialog->ad2Edit->text()+registrationDialog->ad3Edit->text(),
                                                        registrationDialog->firstNameEdit->text(), registrationDialog->middleNameEdit->text(), registrationDialog->lastNameEdit->text(),
                                                        registrationDialog->pContactEdit->text(), registrationDialog->sContactEdit->text(), registrationDialog->line1Edit->text(),
                ...
                QTimer::singleShot(100, this, SLOT(completionMessage()));
                OfflineEmpRegistration::registerOffline(eID,"1",registrationDialog->ad1Edit->text()+registrationDialog->ad2Edit->text()+registrationDialog->ad3Edit->text(),
                                                        registrationDialog->firstNameEdit->text(), registrationDialog->middleNameEdit->text(), registrationDialog->lastNameEdit->text(),
                                                        registrationDialog->pContactEdit->text(), registrationDialog->sContactEdit->text(), registrationDialog->line1Edit->text(),
                                                        registrationDialog->line2Edit->text(), registrationDialog->stateBox->currentText(), registrationDialog->pincodeEdit->text(),
...
void EmployeeRegistration::tempDBConnetion()
{
    ...
    localDatabase.setDatabaseName(oDetail.getDatabase());
    localDatabase.setUserName(oDetail.getUserName());
    localDatabase.setPassword(oDetail.getUserPass());
   ...
   //make sure by default database have No any department (A0)
...
void EmployeeRegistration::completionMessage()
{
    ...
    regCompDialog->setValue(registrationDialog->firstNameEdit->text() + " " + registrationDialog->lastNameEdit->text(), registrationDialog->departmentBox->currentText(),
                            registrationDialog->designationEdit->text(), mDetail.getDID());
