...
void AcceptRequest::refreshPressed()
{
    emit transformMe();
}
...
void AcceptRequest::rSelPressed()
{
    ...
    if(counter > 3){
        ...
        QMessageBox::information(this, tr("Selection Exceeded - %1").arg(qApp->applicationName()),
                                 tr("Select only 3 dairies at a time sir!"),
...
void AcceptRequest::transformAndMessage()
{
    rLoad(0);
...
void AcceptRequest::aSelPressed()
{
    ...
    if(!saveData()){
        ...
        QTimer::singleShot(mul*2000, this, SLOT(transformAndMessage()));
        QString hb = "";
        for(int row=0; row<tableWidget->rowCount(); row++){
                ...
                if(myDatabase.isOpen()) {
                    ...
                    if(tmpQuery.size()>0){
                        if(tmpQuery.next()) {
                            ...
                            LocalBranch lbranch;
                            lbranch.setDairyInfo(hb,name,street, city,state,pincode,logo, first, middle, last, pc, sc);
                        }
                    }
                }
            }
...
bool AcceptRequest::saveData()
{
    ...
    for(int row=0; row<tableWidget->rowCount();row++){
        if(statusList.at(row) == 1){
            queryString = "update DairyHB set IsActive = '0' where "
                                     "binary LocalDID = '" + checkList.at(row)->text() +
                                     "' and IsActive = '1'; ";
    ...
    if(isOk){
        ...
        for(int row=0 ; row<tableWidget->rowCount(); row++){
            if(statusList.at(row) != 0){
                ...
                queryString += " update DairyRequestLog set  "
                        ...
                        "' where binary SendDairy = '" + checkList.at(row)->text() + "' and binary ToDairy = '" + mDetail.getDID() + "' and RForHD = '1';";
                queryString += " update DairyRequestLog set  "
                               "AcceptDeniedDate = '" +dateTime +"' where binary SendDairy = '" + checkList.at(row)->text() + "' and binary ToDairy = '" +
                        mDetail.getDID() + "' and RForHD = '1';";
            }
        }
        ...
        if(isOk){
            ...
            for(int row = 0; row < tableWidget->rowCount(); row++){
                if(statusList.at(row) == 1){
                    queryString = "select * from DairyHB where binary LocalDID = '" + checkList.at(row)->text() + "' and "
                                                                                                                  "binary HBID = '" + mDetail.getDID() + "';";
                    //check if the dairy is already a previous higher dairy of local diary or not?
                    if(query.exec(queryString)){
                        if(query.size() > 0){
                            if(query.next()){
                                queryString = " update DairyHB set IsActive  = '1' where binary LocalDID = '" + checkList.at(row)->text() + "' and "
                                                                                                                                            "binary HBID = '" + mDetail.getDID() + "';";
                                if(!query.exec(queryString)){
                                    isOk = false;
                                    //finish transaction, show error message and return ;
                            ...
                            queryString = " insert into DairyHB (LocalDID, HBID, IsActive ) values ('" + checkList.at(row)->text() + "', '" + mDetail.getDID() + "', '1');";
                            if(!query.exec(queryString)){
                                isOk = false;
                                //finish transaction, show error message and return;
                            }
...
void AcceptRequest::sendMail()
{
    for(int row=0; row<tableWidget->rowCount(); row++){
        if(statusList.at(row) == 1){
            QString subject = "Request Accepted!";
            QString message = tr("Congratulations ! \n\nDear %1,\n This is %2 and we are very happy to inform you that your request of being our local"
                                 " branch is accepted.\n"
        ...
        } else if(statusList.at(row) == 2){
            QString subject = "Request Denied!";
            QString message = tr("Dear %1,\n This is %2 and we are very sorry to inform you that your request of being our local"
                                 " branch is denied due to some reasons.\n"
                                 "For further details, please call us on +91 %3").arg(checkList.at(row)->text()).arg(mDetail.getDID()).arg(mDetail.getpc());
...
void AcceptRequest::rLoad(int i)
{
    if(i == 1){
        tableWidget->setEnabled(false);
...
void AcceptRequest::paintEvent(QPaintEvent *event)
{
